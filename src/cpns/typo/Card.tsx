...
const Card = ({ mainTypo, subText, bg, id }: CardProps) => {
  ...
  return (
    <>
      <Image src={bg} alt={mainTypo} className="bg_img z-10 absolute top-0 left-0 size-full object-cover" />
      <div ref={dimmedRef} className={`absolute z-20 top-0 left-0 size-full bg-black opacity-0`}></div>
      <div className="relative card_txt z-30 size-full text-white" id="hover_trigger" onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
        <p className="absolute w-full flex items-center justify-center top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-3xl">
          ...
          <span ref={restCharRef} className="inline-block w-0 opacity-0 overflow-hidden text-nowrap ">
            {mainTypo.slice(1)}
          </span>
        </p>
        <p className="absolute left-1/2 -translate-x-1/2 -translate-y-1/2 overflow-hidden" style={{ top: "calc(50% + 32px)" }}>
